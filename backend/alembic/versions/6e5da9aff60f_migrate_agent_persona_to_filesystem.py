...
def upgrade() -> None:
    ...
    # Use try-except to handle cases where tables might not exist or schema is different during dry runs
    try:
        ...
        for agent_id, project_id, name, persona_prompt in agents:
            ...
            if project and persona_prompt:
                ...
                sanitized_name = name.strip().lower().replace(' ', '_').replace('-', '_').replace('/', '_')
                ...
                try:
                    ...
                    # Only write if file doesn't exist to avoid overwriting manual changes if run multiple times
                    ...
                    connection.execute(
                        sa.text("UPDATE agents SET persona_path = :path WHERE id = :id")
                        .bindparams(path=persona_rel_path, id=agent_id)
                    )
...
def downgrade() -> None:
    ...
    try:
        agents = connection.execute(
            sa.text("SELECT id, project_id, persona_path FROM agents WHERE persona_path IS NOT NULL")
        ).fetchall()
            ...
            if project:
                ...
                if persona_full_path.exists():
                    try:
                        ...
                        connection.execute(
                            sa.text("UPDATE agents SET initial_persona_prompt = :content WHERE id = :id")
