...
5. 最近里程碑：
`backend/` 已完成任务状态机、Claude Code 适配、运行恢复与人工干预闭环，`/api/v1/tasks/{id}/run` 可触发执行并写入 run 与状态事件。
...
- 依赖：P3-B step 1
- 串行任务：
1. [ ] 实现 Prompt 组装器：聚合 DB 任务信息、`docs/` 规范与全局规则。
2. [ ] 引入 Token Budget 管理：基于优先级动态裁剪上下文长度。
3. [ ] 集成模板引擎：支持为不同 Phase/任务类型加载不同 Prompt 模板。
4. [ ] 编写上下文生成测试（验证关键约束包含性与 Token 限制）。
...
- 依赖：P4-C step 2
- 串行任务：
1. [ ] 实现 `tasks.md` 生成器：基于 DB 状态渲染人类可读视图。
2. [ ] 建立同步机制：任务状态变更事件触发 `tasks.md` 刷新。
3. [ ] 优化 Markdown 渲染模板，确保与 `docs/` 规范一致。
4. [ ] 编写导出一致性测试（DB 变更后文件内容正确更新）。

Phase 4 验收：
1. [ ] 敏感文件与越权路径访问被有效拦截。
2. [ ] CLI 可接收包含完整上下文的 Prompt 并正确理解任务。
3. [ ] CLI 可通过工具正确回写任务状态至数据库。
4. [ ] `tasks.md` 始终反映数据库中的最新任务状态。
