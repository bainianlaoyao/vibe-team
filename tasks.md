...
12. Phase 10 状态：`规划中（0/6）` - 聊天系统对齐 Claude Code 交互体验（重点：`ask userquestion` / `request_input` 闭环）。
13. 当前主要风险：现有 Chat 前端仅渲染 `assistant.chunk`/`assistant.complete`，工具透明、输入请求、系统事件和问题回填未形成完整闭环。
...
## Phase 10: 聊天系统对齐 Claude Code 交互体验（2026-02-11 新增）
...
- 优先级：高
- 串行任务：
1. [ ] 梳理 `claude-agent-sdk` 消息类型与控制请求（含 `can_use_tool`/`interrupt`）并形成映射矩阵。
2. [ ] 定义 Conversation WS v2 事件契约（统一字段：`conversation_id`、`turn_id`、`sequence`、`timestamp`、`trace_id`）。
3. [ ] 在后端保留 `raw_event`（可选裁剪）以支持调试与问题复盘。
4. [ ] 规范 `message.replay` 的事件重放行为（按 sequence 幂等补发）。
5. [ ] 增加版本协商与 feature flag：`CHAT_PROTOCOL_V2_ENABLED`（默认关闭）。
6. [ ] 产出文档：`docs/reports/phase10/conversation_protocol_v2.md`。
...
- 优先级：最高
- 串行任务：
1. [ ] 统一问题标识：`question_id == tool_use_id`，贯穿 `assistant.request_input` 与 `user.input_response`。
2. [ ] 扩展 `assistant.request_input` payload（`question_id/question/options/required/metadata`）。
3. [ ] 扩展 `user.input_response` payload（`question_id/answer/resume_task`）并校验必填关系。
4. [ ] 后端将输入回答按 `parent_tool_use_id + tool_use_result` 语义回填到 Claude SDK，而非仅作为普通文本消息。
5. [ ] 保持与收件箱联动：可选创建或关联 `await_user_input` 项，支持跨页面继续处理。
6. [ ] 增加异常分支：无效 `question_id`、重复回答、超时未答、会话已中断。
...
- 优先级：高
- 串行任务：
1. [ ] 将“单次 `query + receive_response`”改为“会话级 `ClaudeSDKClient` 长连接 + 多轮消息泵”。
2. [ ] 建立对话级状态机：`active/streaming/waiting_input/interrupted/error`。
3. [ ] `user.interrupt` 对接 SDK 原生 `interrupt()` 控制通道。
4. [ ] 强化并发控制：同一 conversation 仅允许一个活跃执行回合，重复消息进入队列或拒绝。
5. [ ] 补齐断线重连后的恢复策略（session 恢复 + 未完成回合收敛）。
6. [ ] 对接结构化日志，记录 turn 级耗时、错误、取消原因与工具链路。
...
5. [ ] 后端协议与前端渲染在 feature flag 开关下可双轨兼容。
...
关键路径：
1. `P2-B` -> `P3-A` -> `P3-C0` -> `P3-C` -> `P4-C` -> `P6-A` -> `P7-A` -> `P8-A` -> `P8-B` -> `P10-A` -> `P10-B` -> `P10-C` -> `P10-D` -> `P10-E` -> `P10-F`
