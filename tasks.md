...
### 并行任务 P4-A：最小安全文件访问层（配合 Git Worktree）

- Owner: Document Service
- 依赖：Phase 2 完成
- 串行任务：
1. [ ] 固化执行根目录为当前 `git worktree`，工具访问范围限定在该根目录内。
2. [ ] 实现 `realpath/commonpath` 校验，拦截路径穿越和符号链接越界。
3. [ ] 工具保持只读，并限制单次读取大小、文本类型与超时。
4. [ ] 编写最小安全测试（越权路径、symlink 越界、超大文件、二进制文件）。
