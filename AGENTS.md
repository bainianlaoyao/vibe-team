...
11. 开始搜索或分析前，先查阅 `docs/backend_mvp_tech.md` 的“2. 项目现有知识与关键概念”和“13. docs 全量索引（入口）”章节，再按索引定向读取。
...
1. 默认入口固定为 `docs/backend_mvp_tech.md`，先读第 2 节与第 13 节，再决定下一步文档范围。
2. 按任务类型最小化读取：架构与接口优先读 `docs/backend_mvp_tech.md`，实现历史读 `docs/backend_mvp_impl_history.md`，对话能力读 `docs/backend_mvp_conversation.md`，交付运维读 `docs/deployment.md`/`docs/operations.md`/`docs/runbook/phase5_recovery_sop.md`。
3. 文档无法回答时再读代码，先用 `rg` 精确定位符号/路径，再做局部文件读取，避免全仓逐文件扫描。
4. 涉及文档增删改时，必须同步维护两个索引：`docs/backend_mvp_tech.md` 第 13 节与 `docs/README.md`。
5. 新增或删除 `docs/` 文件后，需确保索引链接可用且不留失效路径。
