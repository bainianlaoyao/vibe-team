...
const directiveQueue = ref<Array<{ element: string, instruction: string, timestamp: number }>>([])
...
// Handle directive from preview
const handleDirective = (element: string, instruction: string) => {
  // Find the appropriate agent (design agent for visual changes)
  const targetAgent = agents.value.find(a => a.role === 'design' || a.role === 'frontend')

  if (targetAgent) {
    // Update agent status
    targetAgent.status = 'coding'
    targetAgent.currentAction = `🎯 Applying directive: "${instruction}"`

    // Add to queue
    directiveQueue.value.push({
      element,
      instruction,
      timestamp: Date.now()
    })

    addTerminalLog(`[DIRECTIVE] → ${targetAgent.name}: "${instruction}"`)

    // Simulate agent working
    setTimeout(() => {
      targetAgent.status = 'done'
      targetAgent.currentAction = '✓ Directive applied'

      // Show success toast
      toast.value = {
        message: `✅ ${element} updated by ${targetAgent.name}`,
        type: 'success'
      }
      ...
      addTerminalLog(`[SUCCESS] ${element} updated`)
    }, 2000)
  }
}
...
<template>
  <div class="h-screen flex flex-col">
    ...
    <header class="glass border-b border-white/10 px-6 py-4">
      <div class="flex items-center justify-between">
        <div class="flex items-center gap-4">
          ...
          <div class="ml-4 px-3 py-1 bg-electric-indigo/20 text-electric-indigo rounded-lg text-xs font-medium">
            Interactive Mode
          </div>
        </div>
    ...
    <div class="flex-1 flex overflow-hidden">
      ...
      <aside class="w-2/5 border-r border-white/10 flex flex-col">
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          <!-- Directive Queue -->
          <div v-if="directiveQueue.length > 0" class="mt-6">
            <h3 class="text-sm uppercase tracking-wider text-white/40 mb-3">Recent Directives</h3>
            <div class="space-y-2">
              <div
                v-for="(dir, i) in directiveQueue.slice(-3).reverse()"
                :key="i"
                class="glass p-3 rounded-lg border border-electric-indigo/20 animate-pop-in"
              >
                <div class="text-xs text-electric-indigo mb-1">→ {{ dir.element }}</div>
                <div class="text-sm text-white/80">{{ dir.instruction }}</div>
              </div>
            </div>
          </div>
        </div>
    ...
    <Transition name="toast">
      <div
        v-if="toast"
        class="fixed bottom-8 right-8 px-6 py-4 glass-strong rounded-xl shadow-2xl
               border border-electric-indigo/30 animate-pop-in z-50"
      >
        <div class="flex items-center gap-3">
          <div class="w-8 h-8 bg-neon-green/20 rounded-lg flex items-center justify-center text-neon-green">
            ✓
          </div>
